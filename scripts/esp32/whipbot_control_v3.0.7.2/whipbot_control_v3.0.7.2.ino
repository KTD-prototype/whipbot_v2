...
int pwm_offset_linear = 0, pwm_offset_rotation = 0;
...
void loop() {
  ...
  pwm_output_L = pwm_output_L + pwm_offset_linear + pwm_offset_rotation;
  pwm_output_R = pwm_output_R + pwm_offset_linear - pwm_offset_rotation;
  ...
  if (COM_FLAG == true) {
    if (Serial.available() >= 3) {
      if (Serial.read() == 'H') {
        target_angle = 0.001 * (float(receive_motion_target()));
        pwm_offset_linear = receive_motion_target();
        pwm_offset_rotation = receive_motion_target();

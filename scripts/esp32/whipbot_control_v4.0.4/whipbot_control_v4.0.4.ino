...
bool flag_voltage_low = false; //whether battery voltage are low than a certain value
bool voltage_low_led = false; //HIGH/LOW to make a LED blink
...
void loop() {
  ...
  //  calculate accumulated error only when Ki is not zero
  if (Ki_posture != 0) {
  ...
  //  calculate base PWM value for motor (at this step, L/R has same value)
  ...
  //  add or distract to control robot's heading
  ...
  float battery_voltage = raw_voltage * 3.3 * 4 / 4096;

  // check whether motors can be driven, based on battery voltage and robot's posture angle
  bool drive_flag = motor_drive_enable(battery_voltage);

  // if the check was clear
  if (drive_flag == true) {
    // set HIGH the enable pin of the motor driver module
    digitalWrite(DISABLE_L, HIGH);
    digitalWrite(DISABLE_R, HIGH);

    // ramp and limit pwm output value
  ...
  passed_time_led += micros() - last_time;
  last_time = micros();
  ...
  if (COM_FLAG == true) {
    ...
    if (passed_time_com > 10000) {
      //      write code for monitoring for debug.
    }
  }

  if (battery_voltage < 10.5) {
    if (passed_time_led > 500000) {
